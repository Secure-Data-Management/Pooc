...
def mPECK(pk_list, W, params, message=""):
    """
    multi Public key Encryption with Conjuctive Keyword
    Performs the encryption of the keywords, using the mPECK model. Encrypts the keywords in W using the public keys in pk_list
    Parameters:
        pk: list of keys (from Keygen.keys)
        W: set of keywords
        params: a dictionary containing the security parameters
    Returns:
        [E, S] as in the paper, E=b"" if there was no message
            S = [A, B, C] with A=g^r, B=[B1, ..., Bn] and C=[C1, ..., Cl]
    """
    ...
    #     E = hash_G2_to_M(params["e"](params["g"], params["g"]) ** (r*s))
    #     print(E)
...
if __name__ == "__main__":
    ...
    mPECK(pk_list, ["test", "encryption"], params, message='This is the message')